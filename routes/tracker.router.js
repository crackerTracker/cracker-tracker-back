...
router.get(
    ...
    async (req, res) => {
        ...
        try {
            ...
            *   totalEarlierDays: 89, // всего дней, начиная с первого дня запрашиваемого месяца, в порядке к более ранним
            ...
            * */
            ...
            // timestamp даты типа <год>-<месяц>-1T00:00:00.000Z; Date.UTC на всякий случай
            const startDayTimestamp = new Date(Date.UTC(year, month, 1, 0, 0, 0, 0)).getTime();

            // коллекции просто чтобы не держать массив, по которому нужно пробегаться
            // ранние дни (с первой даты запрашиваемого месяца включительно по более ранние дни)
            const earlierDaysMap = new Set();
            // поздние дни (от последнего дня месяца, следующего за запрашиваемым, включительно по более поздние)
            const laterDaysMap = new Set();

            // получить задачи с дней, начиная с первого дня запрашиваемого месяца по более ранние,
            ...
            while (collectedDays < daysAmount) {
                ...
                // вычисляю следующий месяц и год (то есть более ранние)
